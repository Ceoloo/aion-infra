...
# Every query failure (docker, psql, SQL) is normalised to exit 2, so 1 always means "overdue" and never "could not query".
q() { docker exec "${CONTAINER}" psql -U "${DBUSER}" -d "${DB}" -X -tA -F '|' -v ON_ERROR_STOP=1 -c "$1" || { echo "[unclassified-missions] query failed" >&2; exit 2; }; }
...
row="$(q "SELECT unclassified_count, coalesce(oldest_unclassified_created_at::text,''), coalesce(last_classification_change_at::text,'') FROM ol_metrics.classification_health")"
IFS='|' read -r unclassified oldest last_change <<<"${row}"
echo "[unclassified-missions] unclassified=${unclassified} (excluded from KPIs until classified)  oldest_unclassified=${oldest:--}  last_classification_change=${last_change:--}"
if [ "${unclassified}" -gt 0 ]; then
  listing="$(q "SELECT mission_id, created_at, status, coalesce(cohort_hint,'-'), coalesce(console_production_flag,'-'), coalesce(console_synthetic_flag,'-') FROM ol_metrics.unclassified_missions ORDER BY created_at")"
  printf '%s\n' "${listing}" \
