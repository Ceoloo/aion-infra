-- PROPOSED (NOT APPLIED to production). Fixes two reporting-integrity gaps found 2026-09-20:
--  1. mission_record defaulted every mission with no mission_context row to cohort 'OL-001', ignoring the
--     mission's own metadata.cohort -> the 5 pre_ol_validation missions were counted inside OL-001 KPIs.
--  2. Nothing excluded synthetic/non-production records from business-value KPIs (economic_value falls back to
--     executions.revenue_attributed, which synthetic proofs populate and which carries no marker).
-- cohort_kpis now counts a mission ONLY if metadata.productionEconomic='true' AND synthetic is not 'true' AND it
-- is not a proof record (fail-closed: unflagged missions are excluded, not included). mission_record keeps listing
-- every mission for operational views. Both are CREATE OR REPLACE with unchanged column lists: rollback is
-- sql/ol-metrics-rollback.sql (exact prior definitions). No table, data or grant changes.
BEGIN;
...
CREATE OR REPLACE VIEW ol_metrics.cohort_kpis AS
 SELECT cohort,
  ...
  WHERE mission_id IN ( SELECT missions.mission_id
           FROM public.missions
          WHERE (missions.metadata ->> 'productionEconomic') = 'true'
            AND COALESCE(missions.metadata ->> 'synthetic', 'false') <> 'true'
            AND missions.metadata ->> 'proof' IS NULL)
  GROUP BY cohort;
COMMIT;
