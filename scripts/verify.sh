...
if command -v terraform >/dev/null; then
  ...
  for d in bootstrap staging production; do
    ( cd "providers/gcp/terraform/environments/$d" && terraform init -backend=false -input=false >/dev/null 2>&1 \
        && terraform validate >/dev/null 2>&1 ) || vfail=1
  done
  [ "$(cd providers/gcp/terraform && terraform fmt -check -recursive >/dev/null 2>&1; echo $?)" = 0 ] || vfail=1
  ...
  find providers/gcp/terraform -name .terraform -type d -prune -exec rm -rf {} + 2>/dev/null
else
  note IAC_VALIDATE "terraform not installed — skipped"
fi

# SECRET_REFERENCE_VALIDATION ------------------------------------------------
if has 'secret_key_ref' providers/gcp/terraform/modules/runtime && has 'google_secret_manager_secret' providers/gcp/terraform/modules/secrets; then
...
if has 'google_cloud_run_v2_job' providers/gcp/terraform/modules/runtime/main.tf \
   && has 'dl.migrate()' runtime/src/migrate.ts && [ -f providers/gcp/scripts/migrate.sh ]; then
...
if has '/health/live' runtime/src/server.ts && has '/health/ready' runtime/src/server.ts \
   && has 'liveness_probe' providers/gcp/terraform/modules/runtime/main.tf && has 'startup_probe' providers/gcp/terraform/modules/runtime/main.tf; then
...
# RESTORE_RUNBOOK ------------------------------------------------------------
if [ -x providers/gcp/scripts/backup-verify.sh ] && has "isolated restore target" providers/gcp/scripts/backup-verify.sh \
...
has 'output "database_url"' providers/gcp/terraform/modules/secrets || true
